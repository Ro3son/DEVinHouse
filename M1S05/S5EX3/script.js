document.body.setAttribute('style', 'font-family: courier; background-color: #1a1a1b; color: #d7dadc');
...
input.setAttribute('style', 'width: 200px; padding: 8px; font-size: 1em; background-color: transparent; color: #d7dadc; border-radius: 10px; border: 2px solid #ff4500; outline: none');
...
const requestURL = 'https://servicodados.ibge.gov.br/api/v1/censos/nomes/ranking?qtd=1000';
...
const buscaNome = (armazenaNomes) => {
    const filtraNome = armazenaNomes.filter((objeto) => {
        return objeto.nome.includes(input.value); // retorna true or false //
    });
    if (ul != null) {
